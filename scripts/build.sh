...
EOF
    ...
    hdiutil create -volname "CScode" \
        -srcfolder "$APP_DIR" \
        -ov -format UDZO \
        "$DIST/CScode-0.3.6-macos.dmg" 2>/dev/null || \
        echo "DMG creation skipped (requires macOS)"

    echo "macOS package: $DIST/CScode-0.3.6-macos.dmg"
}
...
package_linux() {
    ...
    local PKG_DIR="$DIST/linux/cscode-0.3.6"
...
EOF
    ...
    # Create tar.gz
    cd "$DIST/linux"
    tar -czf "$DIST/CScode-0.3.6-linux-x64.tar.gz" "cscode-0.3.6"
    cd "$ROOT"

    echo "Linux package: $DIST/CScode-0.3.6-linux-x64.tar.gz"
}
...
package_windows() {
    ...
    cd "$DIST/windows"
    zip -r "$DIST/CScode-0.3.6-windows-x64.zip" cscode.exe 2>/dev/null || \
        echo "zip failed (install zip utility)"
    cd "$ROOT"

    echo "Windows package: $DIST/CScode-0.3.6-windows-x64.zip"
}
